/* scene creation: setup particles, attractors computation */
import * as THREE from 'three/webgpu';

import {
    float, If, PI, color, cos, instanceIndex,
    Loop, mix, mod, sin, instancedArray, Fn,
    uint, uniform, uniformArray, hash, vec3, vec4
} from 'three/tsl';
...
} from './motionDetector.js';


let camera, scene, renderer, controls, updateCompute; video;

export async function init() {

    camera = new THREE.PerspectiveCamera( 25, window.innerWidth / window.innerHeight, 0.1, 100 );
    camera.position.set( 3, 4, 5 );
    camera.userData.phi = 0.0;
    ...
    video_tex.colorSpace = THREE.SRGBColorSpace;
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    canvas.width = 160;
    canvas.height = 120;
    const motionDetector = new MotionDetector(video);
    let smoothedLeft = 0;
    let smoothedRight = 0;
    setInterval(() => {
        motionDetector.update(ctx, canvas.width, canvas.height);
        const left = motionDetector.getLeftMotion();
        const right = motionDetector.getRightMotion();


        smoothedLeft = smoothedLeft * 0.7 + left * 0.3;
        smoothedRight = smoothedRight * 0.7 + right * 0.3;

        // Map left motion -> speed
        maxSpeed.value = THREE.MathUtils.clamp(
            smoothedLeft * 0.5, 1, 10
        );

        // Map right motion -> exponent
        // const exponent = THREE.MathUtils.clamp(
        //     Math.floor(right * 0.01), 1, 10
        // );
        // attractorMass.value = Number(`1e${exponent}`);
        spinningStrength.value = THREE.MathUtils.clamp(
            smoothedRight * 0.9, 1.5, 4
        );
        console.log("L "+maxSpeed.value.toFixed(2)+"\tR "+spinningStrength.value.toFixed(2))


    }, 500);   // update every 0.8 seconds
    ...
    const helpersArrowGeometry = new THREE.ConeGeometry( 0.05, 0.2, 6, 1, false );
    ...
    for ( let i = 0; i < attractorsPositions.array.length; i ++ ) {
        ...
        // reinit hidden controls for optics
        attractor.controls.showX = false;
        attractor.controls.showY = false;
        attractor.controls.showZ = false;
    ...
    gui.add( { controlsVisible: attractors[ 0 ].controls.showX }, 'controlsVisible' ).onChange( value => {

        for ( const attractor of attractors ) {

            attractor.controls.showX = value;
            attractor.controls.showY = value;
            attractor.controls.showZ = value;
